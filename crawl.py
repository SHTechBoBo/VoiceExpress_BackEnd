...
def get_comment(topic: str, id_list: list, max_comment: int = 1000, max_threads=os.cpu_count()*2):
    comment_dict = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
...
def download_json(topic: str, comment_dict: dict):
    ...
    # 将处理过的数据保存为JSON文件
    json_path = f"data/comments/{topic}.json"
...
def deal_json(comment_dict: dict):
    for name, comment_info in comment_dict.items():
        # 移除"来自"字符串并更新source
        comment_info["source"] = comment_info["source"].replace("来自", "")
...
def topic_crawl(key: str, num: int = 1000):
    ...
    return deal_json(comment_info_dict)
